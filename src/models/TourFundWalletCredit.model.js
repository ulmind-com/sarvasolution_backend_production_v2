...
/**
 * TourFundWalletCredit
 * -------------------------------------------------
 * Per-user per-month payout audit log for Tour Fund.
 * finalUnits has no cap — can be any integer >= 1.
 */
const tourFundWalletCreditSchema = new mongoose.Schema({
    ...
    poolId:       { type: mongoose.Schema.Types.ObjectId, ref: 'TourFundPool', required: true },
...
tourFundWalletCreditSchema.index({ userId: 1, year: 1, month: 1 }, { unique: true });

const TourFundWalletCredit = mongoose.model('TourFundWalletCredit', tourFundWalletCreditSchema);
export default TourFundWalletCredit;
