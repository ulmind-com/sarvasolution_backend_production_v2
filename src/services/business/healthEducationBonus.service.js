...
import HealthEducationBonusState from '../../models/HealthEducationBonusState.model.js';
...
// ── Constants — 5% Pool, 50,000 BV per Unit, No Unit Capping ────────────────
const POOL_PERCENT     = 0.05;   // 5% of company BV
const BV_PER_UNIT      = 50000;  // 50,000 BV = 1 unit
...
export const healthEducationBonusService = {

    /**
     * Calculate a single user's Health & Education Bonus unit count.
     * 50,000 BV = 1 unit. No capping.
     */
    calculateUserUnits: async (userId, year, month, _prebuiltLookup = null) => {
        ...
        // Fresh leg BV (in-memory traversal)
        ...
        // Add carry-forward
        const state      = await HealthEducationBonusState.findOne({ userId }).lean();
    ...
    runMonthEndDistribution: async (year, month) => {
        console.log(chalk.magenta(`[HealthEducation] Starting distribution for ${year}-${month}...`));

        const existing = await HealthEducationBonusPool.findOne({ year, month });
        if (existing && existing.status === 'distributed') {
            console.warn(chalk.yellow(`[HealthEducation] Already distributed for ${year}-${month}.`));
            return existing;
        }
        ...
        if (companyTotalBV === 0) {
            return HealthEducationBonusPool.findOneAndUpdate(
                { year, month },
        ...
        for (const u of allUsers) {
            const calc = await healthEducationBonusService.calculateUserUnits(u._id, year, month, lookup);
            ...
            await HealthEducationBonusState.findOneAndUpdate(
                { userId: u._id },
                {
                    userId:              u._id,
                    memberId:            u.memberId,
        ...
        if (existing) {
            ...
            pool = await HealthEducationBonusPool.create({
        ...
        for (const { user, calc } of userResults) {
            ...
            await HealthEducationBonusWalletCredit.findOneAndUpdate(
    ...
    applyWalletCredits: async (year, month) => {
        const pool = await HealthEducationBonusPool.findOne({ year, month });
        if (!pool || pool.status !== 'pending') return;

        const credits    = await HealthEducationBonusWalletCredit.find({ year, month, creditedAt: null });
    ...
    getUserStatus: async (userId) => {
        ...
        const User  = (await import('../../models/User.model.js')).default;
        const lookup = await getTreeLookup(User);
        const calc  = await healthEducationBonusService.calculateUserUnits(userId, year, month, lookup);
        const state = await HealthEducationBonusState.findOne({ userId }).lean();
    ...
    getUserHistory: async (userId) =>
        HealthEducationBonusWalletCredit.find({ userId }).sort({ year: -1, month: -1 }).lean(),
    ...
    getUserLiveEstimate: async (userId) => {
        ...
        if (!myCalc) myCalc = await healthEducationBonusService.calculateUserUnits(userId, year, month, lookup);
    ...
    getAdminPoolList: async (page = 1, limit = 12) => {
        ...
        const [pools, total] = await Promise.all([
            HealthEducationBonusPool.find().sort({ year: -1, month: -1 }).skip(skip).limit(limit).lean(),
            HealthEducationBonusPool.countDocuments()
        ]);
        return { pools, total, page, limit };
    },

    getAdminPoolDetails: async (year, month) => {
        const pool = await HealthEducationBonusPool.findOne({ year, month }).lean();
        if (!pool) return null;
        const credits = await HealthEducationBonusWalletCredit.find({ year, month })
    ...
    getAdminUserDetails: async (memberId) => {
        ...
        const [credits, state] = await Promise.all([
            HealthEducationBonusWalletCredit.find({ userId: user._id }).sort({ year: -1, month: -1 }).lean(),
            HealthEducationBonusState.findOne({ userId: user._id }).lean()
        ]);

        const currentMonthCalc = await healthEducationBonusService.calculateUserUnits(
            user._id,
