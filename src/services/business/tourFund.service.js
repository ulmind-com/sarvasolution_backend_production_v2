...
import TourFundState from '../../models/TourFundState.model.js';
...
// ── Constants — 6% Pool, 25,000 BV per Unit, No Unit Capping ─────────────────
const POOL_PERCENT     = 0.06;   // 6% of company BV
const BV_PER_UNIT      = 25000;  // 25,000 BV = 1 unit
...
export const tourFundService = {
    ...
    calculateUserUnits: async (userId, year, month) => {
        ...
        // Step 4: Unit calculation — floor(weakerLeg / 25000), NO cap
    ...
    runMonthEndDistribution: async (year, month) => {
        console.log(chalk.magenta(`[TourFund] Starting month-end distribution for ${year}-${month}...`));

        const existing = await TourFundPool.findOne({ year, month });
        if (existing && existing.status === 'distributed') {
            console.warn(chalk.yellow(`[TourFund] Already distributed for ${year}-${month}. Skipping.`));
            return existing;
        }
        ...
        console.log(chalk.cyan(`[TourFund] Company BV for ${year}-${month}: ${companyTotalBV}`));

        if (companyTotalBV === 0) {
            return TourFundPool.findOneAndUpdate(
                { year, month },
        ...
        console.log(chalk.cyan(`[TourFund] Computing units for ${allUsers.length} users...`));
        ...
        for (const u of allUsers) {
            ...
            await TourFundState.findOneAndUpdate(
                { userId: u._id },
                {
                    userId:              u._id,
                    memberId:            u.memberId,
        ...
        if (existing) {
            ...
            pool = await TourFundPool.create({
        ...
        if (userResults.length === 0) {
            console.warn(chalk.yellow(`[TourFund] No eligible users for ${year}-${month}.`));
            return pool;
        }
        ...
        for (const { user, calc } of userResults) {
            ...
            await TourFundWalletCredit.findOneAndUpdate(
        ...
        console.log(chalk.green(
            `[TourFund] Staged for ${year}-${month}. ${userResults.length} users, ${totalUnits} units, pool ₹${poolAmount.toFixed(2)}.`
        ));
        return pool;
    },
    ...
    applyWalletCredits: async (year, month) => {
        console.log(chalk.magenta(`[TourFund] Applying wallet credits for ${year}-${month}...`));

        const pool = await TourFundPool.findOne({ year, month });
        if (!pool || pool.status !== 'pending') {
            console.warn(chalk.yellow(`[TourFund] No pending pool for ${year}-${month}. Skipping.`));
            return;
        }

        const credits    = await TourFundWalletCredit.find({ year, month, creditedAt: null });
        ...
        for (const credit of credits) {
            ...
            await credit.save();
            console.log(chalk.green(`[TourFund] Credited ₹${credit.netCredit.toFixed(2)} to ${credit.memberId}`));
        }

        pool.status        = 'distributed';
        pool.distributedAt = creditedAt;
        await pool.save();
        console.log(chalk.green(`[TourFund] ✅ All credits applied for ${year}-${month}.`));
    },
    ...
    getUserHistory: async (userId) =>
        TourFundWalletCredit.find({ userId }).sort({ year: -1, month: -1 }).lean(),
    ...
    getAdminPoolList: async (page = 1, limit = 12) => {
        ...
        const [pools, total] = await Promise.all([
            TourFundPool.find().sort({ year: -1, month: -1 }).skip(skip).limit(limit).lean(),
            TourFundPool.countDocuments()
        ]);
        return { pools, total, page, limit };
    },

    getAdminPoolDetails: async (year, month) => {
        const pool = await TourFundPool.findOne({ year, month }).lean();
        if (!pool) return null;
        const credits = await TourFundWalletCredit.find({ year, month })
    ...
    getAdminUserDetails: async (memberId) => {
        ...
        const [credits, state] = await Promise.all([
            TourFundWalletCredit.find({ userId: user._id }).sort({ year: -1, month: -1 }).lean(),
            TourFundState.findOne({ userId: user._id }).lean()
        ]);

        const currentMonthCalc = await tourFundService.calculateUserUnits(
            user._id,
