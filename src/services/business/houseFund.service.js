...
import HouseFundState from '../../models/HouseFundState.model.js';
...
// ── Constants — 3% Pool, 2,50,000 BV per Unit, No Unit Capping ────────────────
const POOL_PERCENT     = 0.03;   // 3% of company BV
const BV_PER_UNIT      = 250000; // 2,50,000 BV = 1 unit
...
const _getCycleQuery = (cycleYear, cycleNumber) => {
    if (cycleNumber === 1) {
        ...
        // October - March (spans into next year)
        return {
            $or: [
...
const _sumRepurchaseBVCycle = async (SelfRepurchaseBVEntry, userIds, cycleYear, cycleNumber) => {
    ...
    const query = { userId: { $in: userIds }, ..._getCycleQuery(cycleYear, cycleNumber) };
...
export const houseFundService = {

    /**
     * Determines the current active cycle based on the date.
     * Cycle 1: Apr 1 - Sept 30 (returns cycleYear = currentYear, cycleNumber = 1)
     * Cycle 2: Oct 1 - Mar 31 (returns cycleYear = currentYear if Oct-Dec, or currentYear-1 if Jan-Mar, cycleNumber = 2)
     */
    getCurrentCycle: () => {
        ...
        if (m >= 4 && m <= 9) {
            return { cycleYear: y, cycleNumber: 1 };
        } else if (m >= 10 && m <= 12) {
            return { cycleYear: y, cycleNumber: 2 };
        } else {
            // Jan, Feb, Mar belong to previous year's Cycle 2
            return { cycleYear: y - 1, cycleNumber: 2 };
        }
    },
    ...
    calculateUserUnits: async (userId, cycleYear, cycleNumber, _prebuiltLookup = null) => {
        ...
        // Add personal BV to weaker leg over the 6-month cycle
        const personalQuery = { userId, ..._getCycleQuery(cycleYear, cycleNumber) };
     ...
     */
    runCycleEndDistribution: async (cycleYear, cycleNumber) => {
        console.log(chalk.cyan(`[HouseFund] Starting half-yearly distribution for Cycle ${cycleYear}-${cycleNumber}...`));

        const existing = await HouseFundPool.findOne({ cycleYear, cycleNumber });
        if (existing && existing.status === 'distributed') {
            console.warn(chalk.yellow(`[HouseFund] Already distributed for Cycle ${cycleYear}-${cycleNumber}.`));
            return existing;
        }
        ...
        if (companyTotalBV === 0) {
            return HouseFundPool.findOneAndUpdate(
                { cycleYear, cycleNumber },
                { companyTotalBV: 0, poolAmount: 0, totalUnits: 0, status: 'held', notes: 'No company BV in this 6-month cycle.' },
        ...
        for (const u of allUsers) {
            ...
            await HouseFundState.findOneAndUpdate(
                { userId: u._id },
                {
                    userId:                   u._id,
                    memberId:                 u.memberId,
        ...
        if (existing) {
            ...
            pool = await HouseFundPool.create({
                cycleYear, cycleNumber, companyTotalBV, poolAmount, totalUnits, perUnitValue,
        ...
        for (const { user, calc } of userResults) {
            ...
            await HouseFundWalletCredit.findOneAndUpdate(
                { userId: user._id, cycleYear, cycleNumber },
                {
                    userId:        user._id,
     ...
     */
    applyWalletCredits: async (cycleYear, cycleNumber) => {
        const pool = await HouseFundPool.findOne({ cycleYear, cycleNumber });
        if (!pool || pool.status !== 'pending') return;

        const credits    = await HouseFundWalletCredit.find({ cycleYear, cycleNumber, creditedAt: null });
    ...
    getUserStatus: async (userId) => {
        ...
        return {
            currentCycle: { cycleYear, cycleNumber },
            carryForward: {
    ...
    getUserHistory: async (userId) =>
        HouseFundWalletCredit.find({ userId }).sort({ cycleYear: -1, cycleNumber: -1 }).lean(),
    ...
    getAdminPoolList: async (page = 1, limit = 12) => {
        ...
        const [pools, total] = await Promise.all([
            HouseFundPool.find().sort({ cycleYear: -1, cycleNumber: -1 }).skip(skip).limit(limit).lean(),
            HouseFundPool.countDocuments()
        ]);
        return { pools, total, page, limit };
    },

    getAdminPoolDetails: async (cycleYear, cycleNumber) => {
        const pool = await HouseFundPool.findOne({ cycleYear, cycleNumber }).lean();
        if (!pool) return null;
        const credits = await HouseFundWalletCredit.find({ cycleYear, cycleNumber })
    ...
    getAdminUserDetails: async (memberId) => {
        ...
        const [credits, state] = await Promise.all([
            HouseFundWalletCredit.find({ userId: user._id }).sort({ cycleYear: -1, cycleNumber: -1 }).lean(),
            HouseFundState.findOne({ userId: user._id }).lean()
        ]);
    ...
    getAdminCurrentCycleOverview: async () => {
        ...
        return { cycleYear, cycleNumber, users: results };
    }
};
